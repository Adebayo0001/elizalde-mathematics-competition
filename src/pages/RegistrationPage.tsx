    ...
    <div className="min-h-screen bg-brand-bg font-sans text-brand-dark flex flex-col">
      ...
      <header className="bg-brand-bg/95 backdrop-blur-md border-b border-slate-200 sticky top-0 z-40">
        <div className="max-w-4xl mx-auto px-6 h-20 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2 text-sm font-bold font-display uppercase tracking-widest text-slate-500 hover:text-brand-dark transition-colors">
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="mb-12">
            <div className="flex justify-between relative">
              ...
              <div 
                className="absolute left-0 top-1/2 -translate-y-1/2 h-1 bg-brand-teal rounded-full z-0 transition-all duration-500 ease-out"
                style={{ width: `${((step - 1) / 3) * 100}%` }}
              />
              
              {steps.map((s) => (
                <div key={s.id} className="relative z-10 flex flex-col items-center gap-2">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-colors duration-300 ${step >= s.id ? 'bg-brand-teal text-white' : 'bg-slate-200 text-slate-400'}`}>
          ...
          {/* Form Content */}
          <div className="bg-white rounded-[2rem] shadow-xl border border-slate-200/50 p-8 md:p-12">
            {step === 1 && (
